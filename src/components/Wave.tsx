...
function Shapes() {
  ...
  const [hovered, setHovered] = useState<"box" | "cylinder" | "pyramid" | null>(
    null
  );
  ...
  useFrame(() => {
    if (!boxMeshRef.current || !pyrMeshRef.current || !cylMeshRef.current)
      return;
  ...
  return (
    <div className="absolute w-screen gridbg">
      <div className="relative w-screen min-h-screen max-w-screen overflow-x-clip overflow-y-visible">
        <div className="absolute top-0 w-screen h-screen -rotate-[36deg] sm:scale-x-150 scale-x-[200%]">
          <WavyBackground
